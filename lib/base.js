...
internals.Base = class {
    ...
    _generate(value, state, prefs) {
        ...
        const id = ids.join(', ');
        const cacheKey = ids.length > 1 ? ids.slice(0, -1).join(', ') : id;
        ...
        if (!state.mainstay.tracer.active &&
            this.$_temp.whens[cacheKey]) {

            return { schema: this.$_temp.whens[cacheKey], id };
        }
    ...
    _values(values, key) {
        ...
        for (const value of values) {
            ...
            const other = key === '_invalids' ? '_invalids' : '_invalids';
            if (obj[other]) {
